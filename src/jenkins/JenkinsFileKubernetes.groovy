podTemplate(containers:[
    ...
    containerTemplate(name: 'maven', image: 'wintermutetec/maven_docker:3.8.1-jdk-11', command: 'sleep', args: '99d'),
  ...
  node(POD_LABEL) {
    stage("Compilacion") {
        container('maven'){
            script{
               sh 'echo "Staring process..."'
			   git url: 'https://github.com/ASG-BPM/cosmonaut-cms',branch:'main',credentialsId: 'winter_user'
			   tag = sh(script:'git describe --tags --always `git rev-list --tags` | grep PROD | head -1',returnStdout: true ).trim()
			   sh "git checkout $tag"
			   sh "mvn package -Ptag=$tag"
		   }
        }
     }
     stage("Push de imagen") {
        container('docker-registry'){
            script{
				sh "/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --cache=true --destination=gcr.io/cosmonaut-uat/cosmonaut/cms:$tag"
			}
        }
     }
        ...
        container('container-registry'){
            script{
		      ...
		      sh "kubectl  --insecure-skip-tls-verify get pods"
		       sh "sed -i 's/\$TAG/$tag/' src/jenkins/cosmonaut-cms_deployment-qa.yml"
		       sh "cat src/jenkins/cosmonaut-cms_deployment-qa.yml | kubectl --insecure-skip-tls-verify  apply -f -"
